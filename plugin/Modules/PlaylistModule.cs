...
namespace MusicBeePlugin.Modules
{
    ...
    public class PlaylistModule : DataModuleBase
    {
        ...
        public PaginatedResponse<Playlist> GetAvailablePlaylists(int limit = 50, int offset = 0, long after = 0)
        {
            ...
            {
                var playlists =
                    db.Select<Playlist>(q => (q.DateAdded > after) || (q.DateDeleted > after) || q.DateUpdated > after);
        ...
        public PaginatedResponse<PlaylistTrack> GetPlaylistTracks(int id, int limit = 50, int offset = 0, long after = 0)
        {
            ...
            {
                var playlistTracks =
                    db.Select<PlaylistTrack>(
                        q =>
                            q.PlaylistId == playlist.Id &&
                            ((q.DateAdded > after) || (q.DateDeleted > after) || q.DateUpdated > after));
