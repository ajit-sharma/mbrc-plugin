namespace MusicBeeRemoteCore
{
    ...
    public partial class Plugin
    {
        ...
        public delegate bool Library_FindDevicePersistentIdDelegate(
            DeviceIdType idType, 
            string[] ids, 
            ref string[] values);
        ...
        public delegate bool Library_GetArtworkExDelegate(
            string sourceFileUrl, 
            int index, 
            bool retrievePictureData, 
            ref PictureLocations pictureLocations, 
        ...
        public delegate bool Library_GetFileTagsDelegate(
            string sourceFileUrl, 
            MetaDataType[] fields, 
            ref string[] results);
        ...
        public delegate bool Library_GetSyncDeltaDelegate(
            string[] cachedFiles, 
            DateTime updatedSince, 
            LibraryCategory categories, 
            ref string[] newFiles, 
            ref string[] updatedFiles, 
        ...
        public delegate string Library_QuerySimilarArtistsDelegate(
            string artistName, 
        ...
        public delegate bool Library_SetDevicePersistentIdDelegate(
            string sourceFileUrl, 
            DeviceIdType idType, 
            string value);
        ...
        public delegate bool MB_DownloadFileDelegate(
            string url, 
        ...
        public delegate bool MB_GetPluginViewInformationDelegate(
            ...
            out string[] viewNames, 
            out string defaultViewName, 
            out WindowState defaultState, 
            out WindowState currentState);

        public delegate bool MB_GetVisualiserInformationDelegate(
            out string[] visualiserNames, 
            out string defaultVisualiserName, 
            out WindowState defaultState, 
            out WindowState currentState);

        public delegate bool MB_InvokeCommandDelegate(Command command, object parameter);

        public delegate bool MB_OpenFilterInTabDelegate(
            MetaDataType field1, 
        ...
        public delegate int Player_OpenStreamHandleDelegate(
            string url, 
            bool useMusicBeeSettings, 
            bool enableDsp, 
        ...
        public delegate bool Player_UpdatePlayStatisticsDelegate(
            string url, 
            PlayStatisticType countType, 
            bool disableScrobble);

        public delegate bool Playlist_AddFilesDelegate(string playlistUrl, string[] filenames);

        public delegate string Playlist_CreatePlaylistDelegate(
            string folderName, 
            string playlistName, 
            string[] filenames);
        ...
        public struct MusicBeeApiInterface
        {
            public void Initialise(IntPtr apiInterfacePtr)
            {
                ...
                if (this.MusicBeeVersion == MusicBeeVersion.v2_0)
                {
                    // MusicBee version 2.0 - Api methods > revision 25 are not available
                    CopyMemory(ref this, apiInterfacePtr, 456);
                }
                else if (this.MusicBeeVersion == MusicBeeVersion.v2_1)
                {
                    CopyMemory(ref this, apiInterfacePtr, 516);
                }
                else if (this.MusicBeeVersion == MusicBeeVersion.v2_2)
                {
                    CopyMemory(ref this, apiInterfacePtr, 584);
                }
                else if (this.MusicBeeVersion == MusicBeeVersion.v2_3)
                {
                    CopyMemory(ref this, apiInterfacePtr, 596);
                }
                else if (this.MusicBeeVersion == MusicBeeVersion.v2_4)
                {
            ...
            public MusicBeeVersion MusicBeeVersion
            {
                get
                {
                    ...
                    else if (this.ApiRevision <= 31)
                    {
                        return MusicBeeVersion.v2_1;
                    }
                    else if (this.ApiRevision <= 33)
                    {
                        return MusicBeeVersion.v2_2;
                    }
                    else if (this.ApiRevision <= 38)
                    {
                        return MusicBeeVersion.v2_3;
                    }
                    else if (this.ApiRevision <= 43)
                    {
